...
p_load("tidyverse", "data.table", "knitr", "rmdformats", "magrittr")
...
# people/places involved on grants in 2017 --------------------------------
people <- people %>%
  select(
    person_id_snsf, 
    institute_place,
  ...
  filter(institute_place != "", !is.na(institute_place)) %>%
  # for consistency
  mutate(projects_as_partner = as.character(projects_as_partner)) %>%
...
# color ramp
make_ramp <- function(alpha1 = .45, alpha2 = .2, 
                      col1 = "#003399", col2 = "#252525", 
                      n_shades = 100) {
...
# map nodes
add_nodes <- function(nodes, ramp, cex_scale = 3) {
  with(nodes, {
    # scale node size
    s <- size /  max(size)
    points(x = x, y = y, 
           pch = 16, cex = cex_scale * sqrt(s), col = ramp[round(100 * s)])
  })
}
# map edges
add_edges <- function(edges, ramp, lwd_scale = 4) {
  with(edges, {
...
par(mar = c(0, 0, 0, 0))
maps::map(database = "worldHires", regions = "Switzerland", fill = FALSE, col = rgb(0, 0, 0, .3))
edge_col <- make_ramp()
add_nodes(nodes_ch, ramp = edge_col)
add_edges(edges_ch, ramp = edge_col)

# label university cities
uni <- c("Zürich", "Lausanne", "Bern", "Genève","Basel", "Fribourg", "Neuchâtel", "St. Gallen", "Lugano", "Luzern")
uni_nodes <- filter(nodes_ch, institute_place %in% uni)
with(uni_nodes, {
  text(x, y, institute_place, 
    pos = 3, cex = .7, col = rgb(0, 0, 0, .66))
})
invisible(dev.off())
```
...
svg("core_eu.svg", width = 12, height = 7)
par(mar = c(0, 0, 0, 0))
...
add_nodes(nodes_eu, ramp = edge_col, cex_scale = 1.2)
add_edges(edges_eu, ramp = edge_col, lwd_scale = 1.2)
invisible(dev.off())
```

**Click to see the map [here](core_eu.svg).**
...
# map nodes
edge_col <- make_ramp(0.5, 0.2)
add_nodes(nodes_io, ramp = edge_col, cex_scale = .5)
